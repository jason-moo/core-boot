...
public class ReflectionUtils {
    ...
    public static Class getSuperClassGenricType(Class clazz, int index) {
        ...
        if (!genType.getClass().isAssignableFrom(ParameterizedType.class)) {
            ...
            if (index < params.length && index >= 0) {
                ...
                logger.warn("Index: {}, Size of {}\'s Parameterized Type: {}", index, clazz.getSimpleName(),
                        params.length);
                return Object.class;
            }
        }
    ...
    public static Object instance(String className) {
        try {
            ...
            logger.error("无法找到方言类", var2);
            return null;
        } catch (InstantiationException var3) {
            logger.error("实例化方言错误", var3);
            return null;
        } catch (IllegalAccessException var4) {
            logger.error("实例化方言错误", var4);
            return null;
        }
    }

    public static RuntimeException convertReflectionExceptionToUnchecked(Exception e) {
        return (RuntimeException) (!(e instanceof IllegalAccessException) && !(e instanceof IllegalArgumentException) && !(e instanceof NoSuchMethodException) ? (e instanceof InvocationTargetException ? new RuntimeException(
                ((InvocationTargetException) e).getTargetException()) : (e instanceof RuntimeException ? (RuntimeException) e : new RuntimeException(
